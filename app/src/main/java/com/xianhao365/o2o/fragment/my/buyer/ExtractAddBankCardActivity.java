...
import com.xianhao365.o2o.entity.wallet.bank.OpenBankEntity;
        ...
        switch (v.getId()){
            case R.id.extract_add_card_detail_btn:
                ...
                if(TextUtils.isEmpty(name)){
                    ...
                    SXUtils.getInstance(activity).ToastCenter("请输入开户行支行");
...
//                SXUtils.getInstance(activity).ToastCenter(jsonBean.get(options1).getChildren().get(option2).getChildren().get(options3).getLabel());
                //返回的分别是三个级别的选中位置
                String tx = openBank.get(options1).getItemLabel();
...
//                .setTitleText("城市选择")//标题
                ...
                .setSubmitColor(R.color.qblue)//确定按钮文字颜色
                .setCancelColor(R.color.qblue)//取消按钮文字颜色
