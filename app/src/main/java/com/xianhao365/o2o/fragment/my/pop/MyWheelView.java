...
/**
 * Created by mfx-t224 on 2017/9/21.
 */
...
public class MyWheelView extends ScrollView {
    ...
    List<AddressProvinceEntity> items;

    private List<AddressProvinceEntity> getItems() {
        return items;
    }

    public void setItems(List<AddressProvinceEntity> list) {
        if (null == items) {
            items = new ArrayList<AddressProvinceEntity>();
        }
        items.clear();
        items.addAll(list);
...
//        }
    ...
    private void initData() {
        ...
        for (AddressProvinceEntity item : items) {
            views.addView(createView(item.getLabel()));
        }
    ...
    private void onSeletedCallBack() {
        if (null != onWheelViewListener) {
            onWheelViewListener.onSelected(selectedIndex, items.get(selectedIndex).getLabel());
        }
    ...
    public String getSeletedItem() {
        return items.get(selectedIndex).getLabel();
    }
